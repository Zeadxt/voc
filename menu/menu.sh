...
Isadmin=$(curl -sS ${voc}/ip | grep $MYIP | awk '{print $5}')
...
Exp=$(curl -sS ${voc}/ip | grep $MYIP | awk '{print $3}')
fi
export RED='\033[0;31m'
...
Exp1=$(curl ${voc}/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
    sts="${Info}"
else
    sts="${Error}"
...
sleep 2
wget -q -O /root/install_up.sh "${url}/menu/install-up.sh" && chmod +x /root/install_up.sh
sleep 2
./install_up.sh
sleep 5
...
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}"
echo -e "$COLOR1┌────────────────────── BY ───────────────────────┐${NC}"
echo -e "$COLOR1 ${NC}              ${WH}• FREE TUNNELING PROJECT •${NC}                $COLOR1 $NC"
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}"
echo ""
...
serverV=$( curl -sS ${url}/menu/versi )
if [ "$Isadmin" = "ON" ]; then
