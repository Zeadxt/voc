...
sed -i '$ i{' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_redirect off;' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_pass http://127.0.0.1:23400;' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_http_version 1.1;' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_set_header X-Real-IP \$remote_addr;' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_set_header Upgrade \$http_upgrade;' /etc/nginx/conf.d/xray.conf
...
cd /usr/bin/
# vmess
wget -O add-vmess "${url}/xray/add-vmess.sh" && chmod +x add-vmess
wget -O trialvmess "${url}/xray/trialvmess.sh" && chmod +x trialvmess
wget -O renew-vmess "${url}/xray/renew-vmess.sh" && chmod +x renew-vmess
wget -O del-vmess "${url}/xray/del-vmess.sh" && chmod +x del-vmess
wget -O cek-vmess "${url}/xray/cek-vmess.sh" && chmod +x cek-vmess
...
wget -O cek-ssws "${url}/xray/cek-ssws.sh" && chmod +x cek-ssws
